...
public abstract class TunTap {
    ...
    /*
     * static void loadLibFromRecsource(String lib, String suffix) throws
     * IOException { File tmp = File.createTempFile("lib", suffix);
     * tmp.deleteOnExit(); InputStream in =
     * TunTap.class.getClassLoader().getResourceAsStream(lib); OutputStream out
     * = new FileOutputStream(tmp);
     *
     * byte[] buffer = new byte[1024*8]; int len;
     *
     * while (0<(len = in.read(buffer))) { out.write(buffer, 0, len); }
     * in.close(); out.close();
     *
